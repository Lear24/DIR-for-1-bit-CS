function [W_eastimated,beta_estimated,objective_value]=...
    onebitCS_invexCentering_Nesterov(beta_initial,X_cell,Y_cell,m,n,d,T_max,lambda)
% [W_eastimated,beta_estimated,objective_value]= onebitCS_invexCentering_Nesterov(beta_initial,X_cell,Y_cell,m,n,d,T_max,lambda)
%
% Optimalize (A) in centering setting.
%
...
for T=1:T_max
    ...
    parfor m_i=1:m
        ...
        if T==1
            ...
            [~ ,grad]= objective_invex_A(v(:,m_i),U_i,X_i,Y_i,m,n_i,d,lambda);
    ...
    if abs(sum(cost_temp)-sum(cost_all_client))<10^(-5)
        ...
        break;
    elseif sum(cost_temp)-sum(cost_all_client)>0.005
