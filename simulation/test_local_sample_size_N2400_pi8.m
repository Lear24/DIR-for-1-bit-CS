...
%% generate the sample size distribution
r_all = zeros(m,n_num);
n_all = zeros(m,n_num);
rng(1)
for i = 1:n_num
...
legend('n_i = N/m','\alpha = 0.3','\alpha = 0.4',...
    '\alpha = 0.5','\alpha = 0.6',...
    '\alpha = 0.7','Location','best')

title('Sorted Sample size Per Node', 'FontSize', 14);
xlabel('Index (Sorted)', 'FontSize', 12);
ylabel('Sample Size', 'FontSize', 12);

% saveas(1,'test_DataDistribution_local_sample_vary_N2400.png')
%%
for rep = 1:rep_num
    for i = 1:n_num
        n = n_all(:,i);
        fprintf('第%d次重复实验, setting of local sample size(alpha) = %d\n',rep,alpha_vary(i));
        rng(rep)
        ...
        save('test_local_sample_vary_N2400_pi8.mat')
    end
end
%%
load('test_local_sample_vary_N2400_pi8.mat')
rep_num = 100;
...
xlim([min(alpha_vary),max(alpha_vary)])
xticks(alpha_vary)
ylim([min(result_mat_cos(:,5))-0.1,1])
% plot(alpha_vary,result_mat_cos(:,1),'-ob','LineWidth',3)
% plot(alpha_vary,result_mat_cos(:,2),'--*r','LineWidth',1.5)
% plot(alpha_vary,result_mat_cos(:,3),'--+g','LineWidth',1)
% plot(alpha_vary,result_mat_cos(:,4),'-o','LineWidth',2,'Color','#4DBEEE')
% plot(alpha_vary,result_mat_cos(:,5),'--g','LineWidth',2)
% plot(alpha_vary,result_mat_cos(:,6),'-oc','LineWidth',2)

plot(alpha_vary, result_mat_cos(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
plot(alpha_vary, result_mat_cos(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(alpha_vary, result_mat_cos(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(alpha_vary, result_mat_cos(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(alpha_vary, result_mat_cos(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形

% title('The local sample size', 'FontSize', 14);
% xlabel('\alpha for local sample size of Dirichlet(\alpha)', 'FontSize', 12);
% ylabel('absolute cosine value', 'FontSize', 12);
% 
% 
% legend('Distributed Invex method','Concentrated Invex method',...
%     'Distributed original method','Least square method',...
%     'Distributed Decoding','LS for one','Location','best','Position',[0.8, 0.9, 0.01, 0.01])
yticks(0:0.1:1);
set(gca, 'FontSize', 20); 
legend('DIR','CIR','SLS','DRD','PLS',...
    'Location','best','Orientation','horizontal','FontSize', 12,'NumColumns', 3,...
    'Position',[0.62,0.65,0.1,0.1])
saveas(1,'test_cos_local_sample_vary_N2400_pi8.png')
...
plot(alpha_vary, result_mat_L2(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
plot(alpha_vary, result_mat_L2(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(alpha_vary, result_mat_L2(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(alpha_vary, result_mat_L2(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(alpha_vary, result_mat_L2(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形

% plot(alpha_vary,result_mat_L2(:,1),'-ob','LineWidth',3)
% plot(alpha_vary,result_mat_L2(:,2),'--*r','LineWidth',1.5)
% plot(alpha_vary,result_mat_L2(:,3),'--+g','LineWidth',1)
% plot(alpha_vary,result_mat_L2(:,4),'-o','LineWidth',2,'Color','#4DBEEE')
% plot(alpha_vary,result_mat_L2(:,5),'--g','LineWidth',2)
% plot(alpha_vary,result_mat_L2(:,6),'-oc','LineWidth',2)
% 
% title('The local sample size', 'FontSize', 14);
% xlabel('\alpha for local sample size of Dirichlet(\alpha)', 'FontSize', 12);
% ylabel('absolute cosine value', 'FontSize', 12);
...
saveas(2,'test_L2_local_sample_vary_N2400_pi8.png')