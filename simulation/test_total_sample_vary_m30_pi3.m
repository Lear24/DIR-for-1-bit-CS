...
for rep = 1:rep_num
    for i = 1:n_num
        ...
        save('test_total_sample_vary_m30_pi3.mat')
    end
end

%%

load('test_total_sample_vary_m30_pi3.mat')
rep_num = 100;
...
% result_mat_cos(end,2) = result_mat_cos(end,1);
% result_mat_L2(end,2) = result_mat_L2(end,1);
...
plot(n_vary, result_mat_cos(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 6);  % 实心星形（红色实线）
hold on;
...
%     'Least square method',...
%     'Distributed Decoding','LS for one','Location','best','FontSize', 7,'Position',[0.7,0.27,0.1,0.1])
% legend('Distributed Invex method','Concentrated Invex method',...
...
saveas(1,'test_cos_total_sample_vary_m30_pi3.png')
...
yticks(0:0.2:4);
set(gca, 'FontSize', 20);
saveas(2,'test_L2_total_sample_vary_m30_pi3.png')
