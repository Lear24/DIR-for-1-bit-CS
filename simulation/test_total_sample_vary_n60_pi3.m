...
d=20; % beta 维数
m_vary = [2;4;8;16;32;64;128;256]; % 7
m_num = 8;
...
beta_estimated_cell = cell (m_num,rep_num,6);
result = cell (m_num,rep_num,6,2);
...
for rep = 1:rep_num
    for i = 1:m_num
        m = m_vary(i);
        n = 60*ones(m,1);
        fprintf('第%d次重复实验, setting of client number(m) = %d\n',rep,m);
        rng(rep)
        ...
        save('test_total_sample_vary_n60_pi3.mat')
    end
end

%%
load('test_total_sample_vary_n60_pi3.mat')
rep_num = 100;
beta_quantail = zeros(m_num,2,4,rep_num);
for i =1 :m_num
    for rep = 1:rep_num
        ...
        beta_quantail(i,2,4,rep) = sum(abs(cell2mat(result(i,rep,1,1))) > abs(cell2mat(result(i,rep,4,1))))/m_vary(i);
    end
end
...
result_mat_cos = zeros(m_num,6);
result_mat_L2 = zeros(m_num,6);
for i = 1:m_num
...
result_mat_L2= log(result_mat_L2);
figure(1)
hold on
grid on
box on
...
xlim([min(log2(m_vary)),max(log2(m_vary))])
xticks(log2(m_vary))
xticklabels({'2^1', '2^2', '2^3','2^4', '2^5', '2^6', '2^7', '2^8'})
ylim([min(result_mat_cos(:,5))-0.1,1])
% plot(log2(m_vary),result_mat_cos(:,1),'-ob','LineWidth',3)
% plot(log2(m_vary),result_mat_cos(:,2),'--*r','LineWidth',1.5)
% % plot(log2(m_vary),result_mat_cos(:,3),'--+g','LineWidth',1)
% plot(log2(m_vary),result_mat_cos(:,4),'-oc','LineWidth',2,'Color','#4DBEEE')
% plot(log2(m_vary),result_mat_cos(:,5),'--g','LineWidth',2)
% plot(log2(m_vary),result_mat_cos(:,6),'-xc','LineWidth',2)

plot(log2(m_vary), result_mat_cos(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
plot(log2(m_vary), result_mat_cos(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(log2(m_vary), result_mat_cos(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(log2(m_vary), result_mat_cos(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(log2(m_vary), result_mat_cos(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形



% title('The total sample size', 'FontSize', 14);
% xlabel('The number of nodes ', 'FontSize', 12);
...
yticks(0:0.1:1);
set(gca, 'FontSize', 20); 
saveas(1,'test_cos_total_sample_vary_n60_pi3.png')
% saveas(1,'test_cos_total_sample_vary_n60_pi3.pdf')
...
% plot(log2(m_vary),result_mat_L2(:,1),'-ob','LineWidth',3)
% plot(log2(m_vary),result_mat_L2(:,2),'-*r','LineWidth',1.5)
% plot(log2(m_vary),result_mat_L2(:,3),'-+g','LineWidth',1)
% plot(log2(m_vary),result_mat_L2(:,4),'-oc','LineWidth',2,'Color','#4DBEEE')
% plot(log2(m_vary),result_mat_L2(:,5),'-oc','LineWidth',2)
% plot(log2(m_vary),result_mat_L2(:,6),'-oc','LineWidth',2)

plot(log2(m_vary), result_mat_L2(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
hold on;
plot(log2(m_vary), result_mat_L2(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(log2(m_vary), result_mat_L2(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(log2(m_vary), result_mat_L2(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(log2(m_vary), result_mat_L2(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形
...
%     'Distributed original method','Least square method',...
%     'Distributed Decoding','LS for one','Location','best')
ylabel('{\bf base-2 logarithm of $l_2$ error}', 'FontSize', 11,'FontWeight','bold','Interpreter','latex');
yticks(-1.5:0.2:0.5);
set(gca, 'FontSize', 20);
saveas(2,'test_L2_total_sample_vary_n60_pi3.png')
% saveas(2,'test_L2_total_sample_vary_n60_pi3.pdf')