...
beta_estimated_cell = cell (sigma2_num,rep_num,6);
result = cell (sigma2_num,rep_num,6,2);
%%
rng('default')
rng(2)
...
% [~, ind] = sort(n);
% ind1 = ind(1:m/2);ind2 = ind(m/2+1:end);
% sigma = sigma1*ones(m,1);q = q1*ones(m,1);
...
for rep = 1:rep_num
    ...
    for i = 1:sigma2_num
        ...
        fprintf('第%d次重复实验, setting variance of noise (sigma) = %d\n',rep,sigma2);
        ...
        save('test_noise_variance_04_rep100_pi8.mat')
    end
end

%%
%
load('test_noise_variance_04_rep100_pi8.mat')
rep_num = 100;
...
sigma2_vary = zeros(sigma2_num,1);
for i =1:sigma2_num
    sigma2_vary(i) = 0.2+0.4*(i-1);
end
figure(1)
hold on
grid on
box on
xticks(sigma2_vary)
xlim([min(sigma2_vary),max(sigma2_vary)])
ylim([min(result_mat_cos(:,5))-0.1,1])

plot(sigma2_vary, result_mat_cos(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
plot(sigma2_vary, result_mat_cos(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(sigma2_vary, result_mat_cos(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(sigma2_vary, result_mat_cos(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(sigma2_vary, result_mat_cos(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形

% plot(sigma2_vary,result_mat_cos(:,1),'-ob','LineWidth',3)
% plot(sigma2_vary,result_mat_cos(:,2),'--*r','LineWidth',1.5)
% % plot(sigma2_vary,result_mat_cos(:,3),'--+g','LineWidth',1)
% plot(sigma2_vary,result_mat_cos(:,4),'-o','LineWidth',2,'Color','#4DBEEE')
% plot(sigma2_vary,result_mat_cos(:,5),'--g','LineWidth',2)
% plot(sigma2_vary,result_mat_cos(:,6),'-oc','LineWidth',2)
% 
% 
% title('The Heterogeneity of Noise', 'FontSize', 14);
% xlabel('The setting of noise variance (\sigma)', 'FontSize', 12);
...
saveas(1,'test_cos_noise_variance_04_rep100_pi8.png')
% save('test_noise_variance_04_rep100_pi3.mat')
...
plot(sigma2_vary, result_mat_L2(:,1), 'b-*','LineWidth',2.5, 'MarkerSize', 10);  % 实心星形（红色实线）
plot(sigma2_vary, result_mat_L2(:,2), 'r-.d', 'LineWidth',1.5,'MarkerSize', 8);  % 蓝色虚线圆圈
plot(sigma2_vary, result_mat_L2(:,4), 'g--o', 'LineWidth',1.5);  % 绿色点划线三角形
plot(sigma2_vary, result_mat_L2(:,5), 'k-.s', 'Marker', 's', 'MarkerFaceColor', 'k', 'DisplayName', 'DRD');  % 黑色点划线方形
plot(sigma2_vary, result_mat_L2(:,6), 'm--v', 'Marker', 'v', 'MarkerFaceColor', 'm', 'DisplayName', 'CIR');  % 品红色点划线菱形

yticks(0:0.2:4);
set(gca, 'FontSize', 20);
legend('DIR','CIR','SLS','DRD','PLS',...
    'Location','best','Orientation','horizontal','FontSize', 12,'NumColumns', 3,...
    'Position',[0.62,0.45,0.1,0.1])
saveas(2,'test_L2_noise_variance_04_rep100_pi8.png')
